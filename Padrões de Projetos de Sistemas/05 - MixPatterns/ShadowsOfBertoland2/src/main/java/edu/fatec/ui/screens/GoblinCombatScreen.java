...
public class GoblinCombatScreen extends TemplateScreen {

    private Image backgroundImage;
    Player selected = GameState.getSelectedPlayer();

    public GoblinCombatScreen() {
        super("Goblin Combat");

        URL imgURL = getClass().getResource("/images/scenarios/goblinScenario.png");
        if (imgURL != null) {
        ...
        ImageIcon goblinIcon = new ImageIcon(getClass().getResource("/images/characters/enemy/goblin.png"));
        Image goblinImg = goblinIcon.getImage().getScaledInstance(290, 290, Image.SCALE_SMOOTH);
        JLabel goblinLabel = new JLabel(new ImageIcon(goblinImg));
        goblinLabel.setBounds(1350, 700, 290, 290);
        backgroundPanel.add(goblinLabel);
        ...
        if (selected != null) {
            ...
            if (characterUrl != null) {
                ...
                switch (selected.getClass().getSimpleName()) {
                    case "Warrior" -> playerLabel.setBounds(180, 590, 390, 390);
                    case "Elf" -> playerLabel.setBounds(180, 599, 390, 390);
                    case "Dwarf" -> playerLabel.setBounds(180, 620, 390, 390);
                    default -> {
                    }
                }
                backgroundPanel.add(playerLabel);
            } else {
                System.err.println("Imagem do personagem não encontrada: " + imagePath);
            }
        }
        add(backgroundPanel);
        ...
        nextButton.addActionListener(e -> {
            ...
            ScreenEventManager.notifyEvent("GOBLIN_COMBAT_FINISHED");
        });
    }
}